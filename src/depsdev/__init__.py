...
class DepsDevClient:
    ...
    async def get_package(self, system: System, name: str) -> Incomplete:
        ...
        """
        return await self._requests(f"/v3/systems/{system}/packages/{url_escape(name)}")

    async def get_version(self, system: System, name: str, version: str) -> Incomplete:
        """
        GetVersion returns information about a specific package version, including its licenses and
        any security advisories known to affect it.

        Example: /v3/systems/npm/packages/%40colors%2Fcolors/versions/1.5.0
        """
        return await self._requests(
            f"/v3/systems/{system}/packages/{url_escape(name)}/versions/{url_escape(version)}"
        )
    ...
    async def get_requirements(self, system: System, name: str, version: str) -> Incomplete:
        ...
        """
        return await self._requests(
            f"/v3/systems/{system}/packages/{url_escape(name)}/versions/{url_escape(version)}:requirements"
        )
    ...
    async def get_dependencies(self, system: System, name: str, version: str) -> Incomplete:
        ...
        Dependencies are currently available for Go, npm, Cargo, Maven and PyPI.

        Dependencies are the resolution of the requirements (dependency constraints) specified by a
        version.

        The dependency graph should be similar to one produced by installing the package version on
        a generic 64-bit Linux system, with no other dependencies present. The precise meaning of
        this varies from system to system.

        Example: /v3/systems/npm/packages/react/versions/18.2.0:dependencies
        """
        return await self._requests(
            f"/v3/systems/{system}/packages/{url_escape(name)}/versions/{url_escape(version)}:dependencies"
        )

    async def get_project(self, project_id: str) -> Incomplete:
        """
        GetProject returns information about projects hosted by GitHub, GitLab, or BitBucket, when
        known to us.

        Example: /v3/projects/github.com%2Ffacebook%2Freact
        """
        return await self._requests(f"/v3/projects/{url_escape(project_id)}")

    async def get_project_package_versions(self, project_id: str) -> Incomplete:
        """
        GetProjectPackageVersions returns known mappings between the requested project and package
        versions. At most 1500 package versions are returned. Mappings which were derived from
        attestations are served first.

        Example: /v3/projects/github.com%2Ffacebook%2Freact:packageversions
        """
        return await self._requests(f"/v3/projects/{url_escape(project_id)}:packageversions")
    ...
    async def get_advisory(self, advisory_id: str) -> Incomplete:
        ...
        """
        return await self._requests(f"/v3/advisories/{url_escape(advisory_id)}")
    ...
    async def query(
        ...
        """
        Query returns information about multiple package versions, which can be specified by name,
        content hash, or both. If a hash was specified in the request, it returns the artifacts that
        matched the hash.

        Querying by content hash is currently supported for npm, Cargo, Maven, NuGet, PyPI and
        RubyGems. It is typical for hash queries to return many results; hashes are matched against
        multiple release artifacts (such as JAR files) that comprise package versions, and any given
        artifact may appear in several package versions.
        ...
        return await self._requests("/v3/query", params=params)  # type: ignore[arg-type,unused-ignore]
